...
#import distiller
#import models
sys.path.append('./mlp_kd_cifar10')
...
if __name__ == "__main__":
    ...
    if args.dataset == 'cifar10':
        ...
        train_loader = torch.utils.data.DataLoader(
                datasets.CIFAR10('../data/CIFAR10_', train = True, download = True, transform = transforms.Compose([
                            transforms.Pad(4, padding_mode = 'reflect'),
                            transforms.RandomCrop(32),
                            transforms.RandomHorizontalFlip(),
                            transforms.ToTensor(),
    ...
    elif args.t_arch =='mlp_mixer_l16_21k':
        config = CONFIGS['Mixer-L_16-21k']
        if args.dataset == 'cifar100':
            teacher_net = MlpMixer(config, 224, num_classes=100, patch_size=16, zero_head=True)
            teacher_net.load_state_dict(torch.load('./classifier_pretrained_weights/Mixer_L16_cifar100_21k_9125_checkpoint.bin'))
            teacher_net.eval()
        elif args.dataset == 'cifar10':
            teacher_net = MlpMixer(config, 224, num_classes=10, patch_size=16, zero_head=True)
            teacher_net.load_state_dict(torch.load('./classifier_pretrained_weights/Mixer-L16_21k_9844_checkpoint.bin'))
            teacher_net.eval()


    elif args.t_arch =='mlp_mixer_l16_1k':
        config = CONFIGS['Mixer-L_16']
        if args.dataset == 'cifar100':
            teacher_net = MlpMixer(config, 224, num_classes=100, patch_size=16, zero_head=True)
            teacher_net.load_state_dict(torch.load('./classifier_pretrained_weights/Mixer_L16_cifar100_1k_8569_checkpoint.bin'))
            teacher_net.eval()
        elif args.dataset == 'cifar10':
            teacher_net = MlpMixer(config, 224, num_classes=10, patch_size=16, zero_head=True)
            teacher_net.load_state_dict(torch.load('./classifier_pretrained_weights/Mixer-L_16_1k_9698_checkpoint.bin'))
            teacher_net.eval()
    ...
    elif args.s_arch == 'mlp_mixer_b16_1k':
        config = CONFIGS['Mixer-B_16']
        if args.dataset == 'cifar10':
            student_net = MlpMixer(config, 224, num_classes=10, patch_size=16, zero_head=True)
            student_net.load_from(np.load('./MLP_Mixer_Pytorch/checkpoint/Mixer-B_16.npz'))
        elif args.dataset == 'cifar100':
            student_net = MlpMixer(config, 224, num_classes=100, patch_size=16, zero_head=True)
            student_net.load_from(np.load('./MLP_Mixer_Pytorch/checkpoint/Mixer-B_16.npz'))


    elif args.s_arch == 'mlp_mixer_b16_21k':
        config = CONFIGS['Mixer-B_16-21k']
        if args.dataset == 'cifar10':
            student_net = MlpMixer(config, 224, num_classes=10, patch_size=16, zero_head=True)
            student_net.load_from(np.load('./MLP_Mixer_Pytorch/checkpoint/imagenet21k_Mixer-B_16.npz'))
        elif args.dataset == 'cifar100':
            student_net = MlpMixer(config, 224, num_classes=100, patch_size=16, zero_head=True)
            student_net.load_from(np.load('./MLP_Mixer_Pytorch/checkpoint/imagenet21k_Mixer-B_16.npz'))
    ...
    # optimizer = optim.AdamW(student_net.parameters(),lr=args.lr)
    optimizer = torch.optim.SGD(student_net.parameters(),
                                lr=args.lr,
                                momentum=0.9,
                                weight_decay=0)
    # scheduler = get_linear_schedule_with_warmup(optimizer, len(train_loader), args.epochs * len(train_loader))
    scheduler = WarmupCosineSchedule(optimizer, warmup_steps=len(train_loader), t_total=args.epochs * len(train_loader))
    best_epoch = 0

    intermediate_dir = './csv_kd/'
    if intermediate_dir:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        pd.DataFrame(acc1).to_csv(os.path.join(intermediate_dir, f'{args.csv_name}.csv'), index=None)
