...
from pyhessian.utils import group_product, group_add, normalization, get_params_grad, get_params_grad_input, hessian_vector_product, orthnormal
...
class hessian():
    ...
    def __init__(self, model, criterion, data=None, dataloader=None, cuda=True):
        ...
        if not self.full_dataset:
            ...
            #print("hessian 68 line , self.inputs dim: ",self.inputs.shape)
            if self.device == 'cuda':
                self.inputs, self.targets = self.inputs.cuda(), self.targets.cuda()
            self.inputs.requires_grad_(True)
            # if we only compute the Hessian information for a single batch data, we can re-use the gradients.
            outputs,_,_,_,_,_ = self.model(self.inputs)
            loss = self.criterion(outputs, self.targets)
            loss.backward(create_graph=True)

        # this step is used to extract the parameters from the model
        #params, gradsH = get_params_grad(self.model)
        params, gradsH = get_params_grad(self.data)
        self.params = params
        self.gradsH = gradsH  # gradient used for Hessian computation
        #print("param shape :",len(params))
        #print("hessian.py self.gradsH :",self.gradsH.shape)
    ...
    def dataloader_hv_product(self, v):
        ...
        for d in self.data:
            ...
            outputs,_,_,_,_,_ = self.model(inputs.to(device))
            loss = self.criterion(outputs, targets.to(device))
            loss.backward(create_graph=True)
            #params, gradsH = get_params_grad(self.model)
            #print("length of inputs: ",len(inputs)) 256

            #print("type of inputs :",type(inputs), "inputs shape :",inputs.shape)
            #inputs = torch.cat(inputs)
            #print("input shape ",inputs.shape)
            #inputs = torch.as_tensor(inputs)
            params, gradsH = get_params_grad(d)
            print("gradsH[0].shape :",gradsH[0].shape)
            #params, gradsH = get_params_grad(d)
            #print("parmas: ",params)
            #print("params shape :",params.shape)
            #self.model.zero_grad()
            #inputs.zero_grad()
            #print("length of param list : ",len(params))
    ...
    def eigenvalues(self, maxIter=100, tol=1e-3, top_n=1):
        ...
        while computed_dim < top_n:
            ...
            for i in range(maxIter):
                ...
                #self.params = [p.zero_grad() for p in self.params]
