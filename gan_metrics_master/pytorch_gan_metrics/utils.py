...
from gan_metrics_master.pytorch_gan_metrics.core import (
...
def class_wise_calc_and_save_stats(
    dataset,
    output_path: str,
    batch_size: int = 256,
    use_torch: bool = False,
    ...
    Args:
        dataset : whole dataset. Will be seperated in classwise.
        output_path (str): Path to the output file.
    ...
    for class_idx, loader in loaders.items():
        acts, = get_inception_feature(
            loader, dims=[2048], use_torch=use_torch, verbose=verbose)
        ...
        output_path2 = output_path + f"{class_idx}.npz"

        if os.path.dirname(output_path2) != "":
            os.makedirs(os.path.dirname(output_path2), exist_ok=True)
        np.savez_compressed(output_path2, mu=mu, sigma=sigma)
