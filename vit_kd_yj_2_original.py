...
if __name__ == "__main__":
    ...
    dataset = load_dataset("cifar100",num_proc=2)

    test_dataset = load_dataset("cifar100",split="test",num_proc=2)
    ...
    def process(examples):
        processed_inputs = teacher_processor(examples["img"])
    ...
    processed_datasets = processed_datasets.remove_columns("coarse_label")    
    processed_datasets = processed_datasets.rename_column("fine_label", "labels")





    processed_test_datasets = test_dataset.map(cifar_process, batched=True)

    # FOR CIFAR100
    processed_test_datasets = processed_test_datasets.remove_columns("coarse_label")    
    processed_test_datasets = processed_test_datasets.rename_column("fine_label", "labels")
    ...
    trainer = ImageDistilTrainer(
        ...
        eval_dataset=processed_test_datasets,
