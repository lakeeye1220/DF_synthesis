...
def load_datasets(set_name, input_size=224):
    if set_name == 'mnist':
        train_dataset = datasets.MNIST(root=os.path.join('data', 'MNIST'),
                                                   train=True,
                                                   transform=transforms.ToTensor(),
                                                   download=True)
        test_dataset = datasets.MNIST(root=os.path.join('data', 'MNIST'),
                                                  train=False,
                                                  transform=transforms.ToTensor())
        ...
        train_dataset = flowers(root='data',
                                  train=True,
                                  val=False,
                                  transform=input_transforms,
                                  download=True)
        test_dataset = flowers(root='data',
                                train=False,
                                val=True,
                                transform=input_transforms,
                                download=True)
